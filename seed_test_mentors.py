#!/usr/bin/env python
"""
Script to seed test mentors in Supabase database
...
def seed_mentors():
    """Insert test mentors into Supabase"""
    print("\n" + "=" * 80)
    ...
    for mentor in test_mentors:
        try:
            # Convert expertise list to JSONB format
            mentor_data = mentor.copy()
            mentor_data['expertise'] = json.dumps(mentor['expertise'])
            
            # Try to insert the mentor
            result = supabase.table('mentors').insert(mentor_data).execute()
            ...
            if 'duplicate key' in error_message.lower() or 'unique constraint' in error_message.lower():
                ...
                print(f"❌ Error creating mentor with user_id {mentor['user_id']}: {error_message}")
    ...
    try:
        result = supabase.table('mentors').select('*').eq('is_approved', True).execute()
        approved_mentors = result.data if result.data else []
        ...
        if approved_mentors:
            ...
            for mentor in approved_mentors[:5]:  # Show first 5
                ...
                if isinstance(expertise_list, str):
                    expertise_list = json.loads(expertise_list)
